...
-- Key sistemi kaldırıldığı için direkt Rayfield menüsünü oluştur
local Window = Rayfield:CreateWindow({
    Name = "SPTS:MODDED Script by LU4DEV👿",
...
local deathLoop = nil -- AutoDeath döngüsünü kontrol etmek için değişken

-- Son konumu güncelleyen fonksiyon
...
task.spawn(updateLastLocation) -- Sonsuz döngüde çalıştır

-- AutoDeath fonksiyonunu aç/kapat
local function toggleAutoDeath(enabled)
    if enabled then
        if deathLoop then return end -- Zaten çalışıyorsa tekrar başlatma
        print("AutoDeath başlatıldı.") -- Kontrol için mesaj
        deathLoop = task.spawn(function()
            while true do
                -- Karakterin doğmasını bekle
                repeat wait() until player.Character and player.Character:FindFirstChildOfClass("Humanoid")

                local humanoid = player.Character:FindFirstChildOfClass("Humanoid")

                -- Humanoid tamamen yüklenene kadar bekle
                repeat wait() until humanoid and humanoid.Health > 0

                -- Oyuncunun öldüğünden emin ol
                while humanoid.Health > 0 do
                    humanoid:TakeDamage(humanoid.MaxHealth) -- Hasar vererek öldür
                    humanoid.Health = 0 -- Emin olmak için tekrar sıfırla
                    wait(0.1)
                end
        ...
        if deathLoop then
            ...
            deathLoop = nil
            print("AutoDeath durduruldu.") -- Kontrol için mesaj
        end
    end
end

-- Ölümden sonra karakteri tekrar yerine getiren fonksiyon
...
task.spawn(respawnLoop) -- Sonsuz döngüde çalıştır
...
local Button = OtherTab:CreateButton({
    Name = "Psy1.1Qid",
    Callback = function()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/massthix/testttt/refs/heads/main/PS20", true))()
    end,
})
